...
/// Configuration to create an [GraycefulEncoding](GraycefulEncoding) layer.
#[derive(Config)]
pub struct GraycefulEncodingConfig {
...
/// Grayceful encoding layer for transformer models.
#[derive(Module, Debug)]
pub struct GraycefulEncoding<B: Backend> {
    encoding: Tensor<B, 3>,
}

impl GraycefulEncodingConfig {
    /// Initialize a new [GraycefulEncoding](GraycefulEncoding) module.
    pub fn init<B: Backend>(&self, device: &B::Device) -> GraycefulEncoding<B> {
        let encoding =
            generate::<B>(self.max_sequence_size, self.d_model, &device).unsqueeze::<3>();

        GraycefulEncoding { encoding }
    }
}

impl<B: Backend> GraycefulEncoding<B> {
    /// Applies the forward pass on the input tensor by adding the grayceful code to the input.
...
/// Returns grayceful code for positional embedding introduced in
...
mod tests {
    ...
    fn test_module() {
        ...
        let pe = GraycefulEncodingConfig::new(d_model).init::<TestBackend>(&device);
