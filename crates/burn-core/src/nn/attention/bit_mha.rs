...
#[derive(Config)]
pub struct BitMultiHeadAttentionConfig {
...
/// # Params
///
/// - query: [Linear](nn::BitLinear) layer with `d_model` input and output features.
/// - key: [Linear](nn::BitLinear) layer with `d_model` input and output features.
/// - value: [Linear](nn::BitLinear) layer with `d_model` input and output features.
/// - output: [Linear](nn::BitLinear) layer with `d_model` input and output features.
#[derive(Module, Debug)]
pub struct BitMultiHeadAttention<B: Backend> {
    query: nn::BitLinear<B>,
    key: nn::BitLinear<B>,
    value: nn::BitLinear<B>,
    output: nn::BitLinear<B>,
    dropout: nn::Dropout,
    activation: nn::Gelu,
    n_heads: usize,
    d_k: usize,
...
#[derive(Debug, Clone)]
pub struct BitMhaInput<B: Backend> {
    query: Tensor<B, 3>,
    key: Tensor<B, 3>,
    value: Tensor<B, 3>,
...
impl BitMultiHeadAttentionConfig {
    ...
    /// [record](BitMultiHeadAttentionRecord).
    pub fn init_with<B: Backend>(
        &self,
        record: BitMultiHeadAttentionRecord<B>,
    ) -> BitMultiHeadAttention<B> {
        let linear = |config: &Self, record| {
            nn::BitLinearConfig::new(config.d_model, config.d_model).init_with(record)
        };
...
#[derive(Debug, Clone)]
pub struct BitMhaOutput<B: Backend> {
...
impl<B: Backend> BitMultiHeadAttention<B> {
    ...
    pub fn forward(&self, input: BitMhaInput<B>) -> BitMhaOutput<B> {
    ...
    fn attention_linear(&self, x: Tensor<B, 3>, linear: &nn::BitLinear<B>) -> Tensor<B, 4> {
...
pub struct BitMhaCache<B: Backend> {
    query: BitMhaLinearCache<B, 4>,
    key: BitMhaLinearCache<B, 4>,
    value: BitMhaLinearCache<B, 4>,
    output: BitMhaLinearCache<B, 3>,
}

enum BitMhaLinearCache<B: Backend, const D: usize> {
    Autoregressive(TensorCache<B, D>, usize),
    Full(TensorCache<B, D>),
}

impl<B: Backend> BitMhaCache<B> {
    /// Initialize a cache for autoregressive inference.
    pub fn autoregressive() -> Self {
        Self {
            query: BitMhaLinearCache::Autoregressive(TensorCache::empty(), 2),
            key: BitMhaLinearCache::Autoregressive(TensorCache::empty(), 2),
            value: BitMhaLinearCache::Autoregressive(TensorCache::empty(), 2),
            output: BitMhaLinearCache::Autoregressive(TensorCache::empty(), 1),
        }
    }
    ...
    pub fn autoregressive_cross_attention() -> Self {
        Self {
            query: BitMhaLinearCache::Autoregressive(TensorCache::empty(), 2),
            key: BitMhaLinearCache::Full(TensorCache::empty()),
            value: BitMhaLinearCache::Full(TensorCache::empty()),
            output: BitMhaLinearCache::Autoregressive(TensorCache::empty(), 1),
        }
    }
}

impl<B: Backend, const D: usize> BitMhaLinearCache<B, D> {
    pub fn forward<F: Fn(Tensor<B, 3>) -> Tensor<B, D>>(
        ...
        match self {
            BitMhaLinearCache::Autoregressive(cache, dim) => {
                cache.forward_autoregressive(tensor, *dim, func)
            }
            BitMhaLinearCache::Full(cache) => cache.forward_full(tensor, func),
        }
    }
}
...
mod tests {
    ...
    fn test_self_attention_shapes() {
        ...
        let mha = BitMultiHeadAttentionConfig::new(d_model, n_heads).init::<TestBackend>(&device);
        let input = BitMhaInput::self_attn(Tensor::random(
    ...
    fn test_generic_mha_shapes() {
        ...
        let mha = BitMultiHeadAttentionConfig::new(d_model, n_heads)
            .init::<TestBackend>(&Default::default());
        let device = Default::default();
        let input = BitMhaInput::new(
            Tensor::random(
    ...
    fn test_self_attention_mask_pad() {
        ...
        let input_1 = BitMhaInput::self_attn(tensor_1).mask_pad(mask_pad.clone());
        let input_2 = BitMhaInput::self_attn(tensor_2).mask_pad(mask_pad);
    ...
    fn test_autoregressive_mask_should_have_same_output_as_autoregressive_decoding() {
        ...
        let input = BitMhaInput::self_attn(tensor.clone()).mask_attn(mask_attn);

        let output_1 = mha.forward(input);
        let mut output_2 = Vec::new();
        let mut cache = BitMhaCache::autoregressive();

        for i in 1..seq_length + 1 {
            let tensor = tensor.clone().slice([0..batch_size, 0..i, 0..d_model]);
            let input = BitMhaInput::self_attn(tensor);
